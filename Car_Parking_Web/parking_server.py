...
MQTT_TOPIC_LICENSE_PLATE_EXIT = "parking/exit/license_plate"
...
def on_connect(client, userdata, flags, rc, properties=None):
    ...
    if rc == 0:
        ...
        client.subscribe(MQTT_TOPIC_LICENSE_PLATE_EXIT)
...
def on_message(client, userdata, msg):
    ...
    if msg.topic == MQTT_TOPIC_LICENSE_PLATE:
    ...
    elif msg.topic == MQTT_TOPIC_LICENSE_PLATE_EXIT:
        license_plate = msg.payload.decode('utf-8')
        ...
        try:
            conn = sqlite3.connect(DB_FILE)
            cursor = conn.cursor()
            cursor.execute("INSERT INTO checkouts (license_plate, checkout_time) VALUES (?, ?)", 
                         (license_plate, checkout_time.strftime("%Y-%m-%d %H:%M:%S")))
            conn.commit()
            conn.close()
            
            # Emit tới dashboard
            socketio.emit('license_plate_exit', {
                'license_plate': license_plate,
                'timestamp': checkout_time.isoformat()
            })
            
            print(f"✅ Lưu biển số rời bãi: {license_plate}")
            
        except Exception as e:
            print(f"❌ Lỗi lưu biển số rời bãi: {e}")
...
def get_checkouts():
    """Lấy lịch sử check-out."""
    try:
        conn = sqlite3.connect(DB_FILE)
        cursor = conn.cursor()
        # Lấy 10 xe ra gần nhất
        cursor.execute("SELECT license_plate, checkout_time FROM checkouts ORDER BY checkout_time DESC LIMIT 10")
        ...
        for checkout in checkouts:
            result.append({
                'license_plate': checkout[0],
                'checkout_time': checkout[1]
            })
            
        return jsonify({'success': True, 'checkouts': result})
    except Exception as e:
        return jsonify({'success': False, 'error': str(e)})
# --- SOCKETIO EVENTS ---
...
if __name__ == '__main__':
    ...
    setup_database()
    # Chỉ chạy MQTT trong tiến trình chính, không chạy trong tiến trình reloader
    if not app.debug or os.environ.get('WERKZEUG_RUN_MAIN') == 'true':
        setup_mqtt()

    print("✅ Server sẵn sàng tại http://172.20.10.3:5000")
    # Thêm use_reloader=False để tắt việc tự khởi động lại gây ra 2 tiến trình
